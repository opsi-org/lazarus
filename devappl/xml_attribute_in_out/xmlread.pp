...
procedure ReadXMLFile(out ADoc: TXMLDocument; f: TStream; const ABaseURI: String; settings: TXMLReaderSettings); overload;
...
procedure ReadXMLFile(out ADoc: TXMLDocument; f: TStream; const ABaseURI: String; settings: TXMLReaderSettings);
var
  Reader: TXMLTextReader;
  ldr: TLoader;
begin
  ADoc := TXMLDocument.Create;
  Reader := TXMLTextReader.Create(f, ABaseURI, settings);
  try
    ldr.ProcessXML(ADoc, Reader);
  finally
    Reader.Free;
