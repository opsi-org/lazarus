...
begin
 try
   {$IFDEF WINDOWS}
   if isUefi then outlist.Append('uefi_mode=true')
   else  outlist.Append('uefi_mode=false');
   {$ENDIF WINDOWS}
   GetBIOSInfo;
   GetSystemInfo;
   GetBaseBoardInfo;
   GetEnclosureInfo;
   if formattype = 'hash' then
   begin
     for i := 0 to outlist.Count -1 do
       writeln(outlist.Strings[i]);
   end;
 except
    on E:Exception do
        Writeln(E.Classname, ':', E.Message);
...
begin
  ...
  if HasOption('format') then begin
    formattype :=  lowercase(trim(GetOptionValue('format')));
    if not ((formattype = 'ini') or (formattype = 'hash')) then
    begin
      myerror := 'Error: No valid format given';
      WriteHelp;
      Terminate;
      Exit;
    end;
  ...
  *)
  ...
  if HasOption('all') then begin
    writeall;
    Terminate;
    Exit;
  end;

  if HasOption('short') then begin
    writeshort;
    Terminate;
    Exit;
  end;
  ...
  (*
  if HasOption('select') then
  begin
    complist := trim(GetOptionValue('select'));
    if complist = '' then
    begin
      myerror := 'Error: No valid component list given';
      WriteHelp;
      Terminate;
    Exit;
    end;
...
begin
  ...
  writeln(paramstr(0));
  writeln(filename);
  writeln('Based on smbios unit by');
  writeln('https://code.google.com/p/tsmbios/');
  writeln('Mozilla Public License Version 1.1');
  writeln('Usage:');
  writeln(filename+ ' [Options]');
  writeln('Options:');
  writeln(' --help -> write this help and exit');
  //writeln(' --format -> hash or ini (default:hash)');
  writeln(' --short -> only "important" info (default)');
  writeln(' --all -> all info ');
  //writeln(' --select --> comma seperated list of components:');
  //writeln('[bios,sysinfo,board,enclosure,processor,portconnetor,sysslot,bioslang,]');
  //writeln('[physmemarray,memdev,memarraymapped,memdevmapped,pointdev,]');
  //writeln('[battery,voltage,cooldev,temp,currelectrical]');
  Terminate;
  halt(-1);
  Exit;
end;
